...
\noindent This document is a reproducible version of the supplement to \citet{ionides21} compiled on \today, using R version \Sexpr{getRversion()}, \code{pomp} version \Sexpr{packageVersion("pomp")} and \code{spatPomp} version \Sexpr{packageVersion("spatPomp")}.
